...
namespace Core.Aspects.Autofac.Caching
{
    public class CacheAspect : MethodInterception
    {
        ...
        {
            var methodName = string.Format($"{invocation.Method.ReflectedType.FullName}.{invocation.Method.Name}");//namespace+
            var arguments = invocation.Arguments.ToList();//parametreler
            var key = $"{methodName}({string.Join(",", arguments.Select(x => x?.ToString() ?? "<Null>"))})";//parametreleri getir//?? varsa solu yoksa sağı ekle
